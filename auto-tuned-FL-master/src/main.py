...
def run(args, seed):
    ...
    # prev_nets for MOON
    prev_local_nets = {}
    models = [global_model for _ in range(args.num_users)]
    best_result = 0.0
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            ...
            if args.alg == 'fedprox':
                w, loss = local_model.update_weights_prox(
                    model=copy.deepcopy(models[idx]), global_round=epoch, mu=args.fedprox_mu)
        ...
        if args.alg == 'feddc':
            if (epoch + 1) % args.dc == 0:
                keys = list(user_groups.keys())
                values = list(user_groups.values())
                random.shuffle(values)
                user_groups = dict(zip(keys, values))
        ...
        if (epoch+1) % print_every == 0:
            ...
            if test_accuracy[-1] > best_result:
                best_result = test_accuracy[-1]
